<script lang="ts">
    ...
    import Emptytab from "../LocalHomeComponents/Emptytab.svelte";
    import TodoItem from "../LocalHomeComponents/TodoItem.svelte";
    import Topbar from "../LocalHomeComponents/Topbar.svelte";
    ...
    const extractCategories = (text: string) => {
        ...
        const categoryList = wordList.filter(
            (item) => item.indexOf("#") !== -1
        );
    ...
    const extractTask = (text: string) => {
        const wordList: Array<string> = text.split(" ");
        const taskWordList = wordList.filter(
            (item) => item.indexOf("#") === -1
        );
